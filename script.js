...
function check(){
	...
	var vache=0;
	var toreau=0;
	if (verifyEntry(userNumber)===false){$("#console").prepend(userNumber+" - Entrada inválida, tente novamente.\r\n")} else {
	for (var n=0; n<4; n++){
		for (var i=0; i<4; i++){
		...
		counter++;
		if (toreau===4){
			$("#console").prepend(userNumber+ " - 4 bulls, VOCÊ GANHOU após "+counter+" tentativas.\r\n"+"Um novo número foi escolhido, jogue novamente..\r\n");
			rnd=computerNumber();
			counter=0;}
		else{$("#console").prepend(userNumber+ " - " + vache +" cow(s) and "+toreau+" bull(s), tentativas: "+counter+"\r\n");}}
		$("#try").val("");
}